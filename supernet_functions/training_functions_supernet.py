...
class TrainerSupernet:
    ...
    def train_loop(self, train_w_loader, train_thetas_loader, test_loader, model):
        ...
        for epoch in range(self.train_thetas_from_the_epoch, self.cnt_epochs):
            ...
            top1_avg, top3_avg, lat_avg, energy_avg = self._validate(model, test_loader, epoch)
    ...
    def _training_step(self, model, loader, optimizer, epoch, info_for_logger=""):
        ...
        for avg in [self.top1, self.top3, self.losses, self.losses_ce, self.losses_lat, self.losses_energy]:
            avg.reset()
    ...
    def _validate(self, model, loader, epoch):
        ...
        for avg in [self.top1, self.top3, self.losses, self.losses_ce, self.losses_lat, self.losses_energy]:
            avg.reset()
        return top1_avg, self.top3.get_avg(), self.losses_lat.get_avg(), self.losses_energy.get_avg() #lat, energy追加
